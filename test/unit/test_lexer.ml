...
let test_simbolos _ =
  ...
  let esperado = [IDENT "contador"; DECL_ASSIGN; INTLIT 1L; EOF] in
  assert_equal esperado (tokenize_string codigo)

let test_selector_con_punto _ =
  let codigo = "fmt.Println(\"ok\")" in
  let esperado =
    [IDENT "fmt"; DOT; STRUCT_ID "Println"; LPAREN; STRINGLIT "ok"; RPAREN; EOF]
  in
  assert_equal esperado (tokenize_string codigo)
...
let test_inserta_semicolon_por_newline _ =
  let codigo = "return\nx := 1\n" in
  let esperado = [RETURN; SEMICOLON; IDENT "x"; DECL_ASSIGN; INTLIT 1L; SEMICOLON; EOF] in
  assert_equal esperado (tokenize_string codigo)

let test_no_inserta_semicolon_tras_operador _ =
  let codigo = "x =\n1\n" in
  let esperado = [IDENT "x"; ASSIGN; INTLIT 1L; SEMICOLON; EOF] in
  assert_equal esperado (tokenize_string codigo)
