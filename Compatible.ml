type celsius = float
and fahrenheit = float
and userid = int
and username = string


let rec celsiustofahrenheit (c : float) : float =
let module Ret = struct exception E of float end in
try
raise (Ret.E ((((c *. 9.) /. 5.) +. 32.)));
0.0
with
| Ret.E v -> v
...
let module Ret = struct exception E of string end in
try
...
let module Ret = struct exception E of bool end in
try
raise (Ret.E (((w = sunday) || (w = saturday))));
false
with
| Ret.E v -> v
...
and rectangle = { mutable topleft : point; mutable bottomright : point; }
and person = { mutable name : string; mutable age : int; mutable id : userid; }


let rec translate (p : point) (dx : float) (dy : float) : point =
let module Ret = struct exception E of point end in
try
raise (Ret.E ({ x = (p.x +. dx); y = (p.y +. dy) }));
Obj.magic ()
with
| Ret.E v -> v
...
let module Ret = struct exception E of float end in
try
raise (Ret.E ((r.bottomright.x -. r.topleft.x)));
0.0
with
| Ret.E v -> v
...
let module Ret = struct exception E of float end in
try
raise (Ret.E ((r.bottomright.y -. r.topleft.y)));
0.0
with
| Ret.E v -> v
...
let module Ret = struct exception E of string end in
try
raise (Ret.E (Printf.sprintf "%s (%s)" a.name a.species));
""
with
| Ret.E v -> v
...
let module Ret = struct exception E of string end in
try
raise (Ret.E (Printf.sprintf "%s, raza %s" (describe (d._embedded)) d.breed));
""
with
| Ret.E v -> v
...
type expr =
  ...
  | V_neg of neg
  | V_let of let_
and num = { mutable value : float; }
and var = { mutable name : string; }
and add = { mutable left : expr; mutable right : expr; }
and mul = { mutable left : expr; mutable right : expr; }
and neg = { mutable e : expr; }
and let_ = { mutable name : string; mutable val_ : expr; mutable body : expr; }
...
raise (Ret.E (v.value))
| V_var v ->
raise (Ret.E ((Hashtbl.find env v.name)))
| V_add v ->
raise (Ret.E ((eval v.left env +. eval v.right env)))
| V_mul v ->
raise (Ret.E ((eval v.left env *. eval v.right env)))
| V_neg v ->
raise (Ret.E ((-. eval v.e env)))
| V_let v ->
let newenv = ref [||] in (
(Hashtbl.iter (fun k val_ -> ignore ((!newenv).(k) = val_)) env);
((!newenv).(v.name) = eval v.val_ env);
raise (Ret.E (eval v.body !newenv))));
raise (Ret.E (float_of_int (0)));
0.0
with
| Ret.E v -> v
...
type tree =
  ...
  | V_tNode of tnode
and leaf = unit
and tnode = { mutable left : tree; mutable value : int; mutable right : tree; }
...
let module Ret = struct exception E of tree end in
try
(match t with
| V_leaf n ->
raise (Ret.E ({ left = V_leaf; value = v; right = V_leaf }))
| V_tNode n ->
(if (v < n.value) then
raise (Ret.E ({ left = insert n.left v; value = n.value; right = n.right })) else
if (v > n.value) then
raise (Ret.E ({ left = n.left; value = n.value; right = insert n.right v })));
raise (Ret.E (n)));
raise (Ret.E (t));
Obj.magic ()
with
...
let module Ret = struct exception E of int array end in
try
(match t with
| V_leaf n ->
raise (Ret.E (()))
...
type intlist =
  ...
  | V_cons of cons
and nil = unit
and cons = { mutable head : int; mutable tail : intlist; }
...
let module Ret = struct exception E of int end in
try
(match l with
| V_nil v ->
raise (Ret.E (0))
| V_cons v ->
raise (Ret.E ((1 + listlen v.tail))));
raise (Ret.E (0));
0
with
| Ret.E v -> v
...
raise (Ret.E (0))
| V_cons v ->
raise (Ret.E ((v.head + listsum v.tail))));
raise (Ret.E (0));
0
with
| Ret.E v -> v
...
let module Ret = struct exception E of (int * bool) end in
try
(for i = 0 to Array.length (xs) - 1 do
      let i = i in
      let x = (xs).(i) in
...
let module Ret = struct exception E of (string, int) Hashtbl.t end in
try
let counts = ref Obj.magic () in (
...
let module Ret = struct exception E of (int * bool) end in
try
let (age, ok) = (Hashtbl.find m name) in (
raise (Ret.E ((!age, !ok))));
(0, false)
with
| Ret.E v -> v
...
let module Ret = struct exception E of 'u array end in
try
let result = ref [||] in (
(for i = 0 to Array.length (xs) - 1 do
      let i = i in
...
let module Ret = struct exception E of 't array end in
try
let result = ref [||] in (
(for i = 0 to Array.length (xs) - 1 do
      let _ = i in
...
let module Ret = struct exception E of 'u end in
try
let acc = ref init in (
(for i = 0 to Array.length (xs) - 1 do
      let _ = i in
...
let module Ret = struct exception E of pair end in
try
raise (Ret.E ({ first = a; second = b }));
Obj.magic ()
with
| Ret.E v -> v

let rec makeadder (n : int) : Obj.t =
let module Ret = struct exception E of Obj.t end in
try
raise (Ret.E ((fun x -> 
raise (Ret.E ((x + n))))));
Obj.magic ()
...
(if (n = 0) then
raise (Ret.E (false)));
raise (Ret.E (iseven (n - 1)));
false
with
| Ret.E v -> v

let rec divide (a : float) (b : float) : (float * Obj.t) =
let module Ret = struct exception E of (float * Obj.t) end in
try
(if (b = 0) then
raise (Ret.E ((float_of_int (0), "división por cero"))));
raise (Ret.E (((a /. b), ())));
(0.0, Obj.magic ())
with
| Ret.E v -> v
...
let module Ret = struct exception E of (int * Obj.t) end in
try
let n = ref 0 in (
let (_, err) = (sscanf) s "%d" (ref !n) in (
(if (!err <> ()) then
raise (Ret.E ((0, Printf.sprintf "no es número: %q" s))));
(if (!n < 0) then
raise (Ret.E ((0, Printf.sprintf "negativo: %d" !n))));
raise (Ret.E ((!n, ())))));
(0, Obj.magic ())
with
| Ret.E v -> v
...
let module Ret = struct exception E of (int * int) end in
try
...
let module Ret = struct exception E of int end in
try
raise (Ret.E (Array.length items s));
0
with
| Ret.E v -> v
...
let module Ret = struct exception E of string end in
try
raise (Ret.E (String.concat " " (Array.to_list [||])));
""
with
| Ret.E v -> v
...
let list = ref { head = 1; tail = { head = 2; tail = { head = 3; tail = V_nil } } } in (
...
let keys = ref [||] in (
(Hashtbl.iter (fun k _ -> ignore (!keys := Array.append !keys [|k|])) !wc);
(());
